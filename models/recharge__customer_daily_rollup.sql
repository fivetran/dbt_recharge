with spine as (
    ...
    select
        *,
        {% for col_name in cols %}
            round(cast(sum({{col_name}}_realized) over (partition by customer_id {{ fivetran_utils.partition_by_source_relation(package_name='recharge') }} order by date_day asc
