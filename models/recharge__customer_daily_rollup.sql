with base as (
    ...
    select
        ...
        count(case when lower(transactions.order_type) = 'recurring' then 1 else null end) as subscription_orders,
        count(case when lower((transactions.order_type) = 'checkout' then 1 else null end) as one_time_orders,
        ...
        {% for col_name in cols %}
            round(sum(case when lower(transactions.order_status) not in ('error', 'skipped', 'queued') 
                then transactions.{{col_name}} else 0 end), 2)
