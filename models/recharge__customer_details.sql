with customers as (
    ...
    select 
        ...
        cast(round({{ dbt.datediff("first_charge_processed_at", dbt.current_timestamp_backcompat(), "day") }} / 30 as {{ dbt.type_numeric() }}), 2)
            as active_months
    ...
    select
        ...
        {% for col in agged_cols %}
            cast(round({{- dbt_utils.safe_divide('total_' ~ col, 'active_months') }} as {{ dbt.type_numeric() }}), 2) as {{col}}_monthly_average -- calculates average over no. active mos
            {{ ',' if not loop.last -}}
        {% endfor %}
