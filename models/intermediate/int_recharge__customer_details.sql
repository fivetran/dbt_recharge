...
-- Agg'd on customer_id
    ...
    select 
        ...
        round(cast(sum(charge_total_price) as {{ dbt.type_numeric() }}), 2) as total_amount_ordered,
        round(cast(avg(charge_total_price) as {{ dbt.type_numeric() }}), 2) as avg_order_amount,
